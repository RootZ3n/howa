...
export async function verifyReceiptEvidence(receipt: DailyDriverReceiptV1, outputRoot: string, verifyLocalRuntime = true): Promise<void> {
  ...
  const authorityRef=receipt.evidence_references.find((ref)=>ref.path.endsWith("trusted-authority.json")); if(!authorityRef) throw new Error("trusted campaign authority evidence missing"); const authority=JSON.parse(await fs.readFile(inside(outputRoot,authorityRef.path),"utf8")) as Record<string,unknown>; if(authority.run_id!==receipt.run_id||authority.trial_id!==receipt.trial_id||authority.fixture_digest!==receipt.fixture_digest||authority.entropy_commitment!==receipt.campaign_entropy_commitment||typeof authority.nonce_hex!=="string"||!/^[a-f0-9]{64}$/.test(authority.nonce_hex)) throw new Error("trusted campaign authority mismatch"); const commitment=sha256(Buffer.concat([Buffer.from("howa-ddv1-entropy-commitment\0"),Buffer.from(authority.nonce_hex,"hex")])); if(commitment!==receipt.campaign_entropy_commitment) throw new Error("campaign entropy commitment is not reproducible");
